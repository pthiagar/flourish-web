...
class EmailDispatcher {
  ...
  getMonthArticles(targetDate = new Date()) {
    ...
    return chosen.length > 0 ? chosen : allPublished.slice(0, 3);
  }
  ...
  generateMonthlyDigestHtml(monthLabel, articles, subscriber) {
    ...
    const articleCardsHtml = articles.map(article => {
      ...
      return `
        <div style="background-color: #FFFFFF; border: 1px solid #DFD2C2; border-radius: 12px; padding: 24px; margin-bottom: 20px; box-shadow: 0 2px 6px rgba(0,0,0,0.02);">
          ...
          <h2 style="font-family: 'Playfair Display', Georgia, serif; font-size: 19px; line-height: 1.35; color: #1A212D; margin: 0 0 10px 0;">
            ${article.title}
          </h2>
          <p style="font-size: 14px; line-height: 1.6; color: #4A5560; margin: 0 0 16px 0;">
            ${article.summary}
          </p>
          <div>
            <a href="${readUrl}" style="display: inline-block; background-color: #1A365D; color: #FAF7F2; text-decoration: none; font-size: 13px; font-weight: 600; padding: 8px 18px; border-radius: 20px;">
              Read Full Letter & Checklist &rarr;
            </a>
          </div>
        </div>
      `;
...
<body style="margin: 0; padding: 0; background-color: #F5EFEB; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; color: #1A212D;">
  <div style="max-width: 640px; margin: 30px auto; background-color: #FAF7F2; border: 1px solid #DFD2C2; border-radius: 16px; overflow: hidden; box-shadow: 0 6px 20px rgba(0,0,0,0.04);">
    ...
    <div style="background-color: #1A365D; padding: 36px 30px; text-align: center; border-bottom: 2px solid #DFD2C2;">
      <h1 style="margin: 0; font-family: 'Playfair Display', Georgia, serif; font-size: 24px; color: #FAF7F2; letter-spacing: 1.5px; font-weight: 400;">
        FLOURISH MANAGEMENT
      </h1>
      <p style="margin: 6px 0 0 0; color: #A5B8D1; font-size: 12px; letter-spacing: 2px; text-transform: uppercase;">
        Executive Monthly Brief • ${monthLabel}
      </p>
    </div>

    <!-- Letter Body -->
    <div style="padding: 36px 30px;">
      <p style="font-size: 15px; line-height: 1.6; color: #1A212D; margin-top: 0;">
        Dear Allocator,
      </p>
      <p style="font-size: 14px; line-height: 1.6; color: #4A5560;">
        Each month, our General Partners publish three prescriptive frameworks focused on capital preservation, physical asset resilience, and venture underwriting discipline. Below is your ${monthLabel} executive brief.
      </p>

      <hr style="border: none; border-top: 1px solid #DFD2C2; margin: 24px 0;">

      <!-- Article Cards -->
      ${articleCardsHtml}

      <div style="background-color: #EFE7DE; border-radius: 12px; padding: 20px; text-align: center; margin-top: 24px;">
        <h3 style="font-family: 'Playfair Display', Georgia, serif; font-size: 16px; color: #1A365D; margin: 0 0 6px 0;">
          Interactive Portfolio Stress-Testing
        </h3>
        <p style="font-size: 13px; color: #4A5560; margin: 0 0 12px 0;">
          Explore real-time downside delta protection and asymmetric alpha capture on our live simulator.
        </p>
  ...
  async dispatchMonthlyDigest(options = {}) {
    ...
    try {
      ...
      const senderEmail = process.env.SENDER_EMAIL || process.env.NOTIFICATION_EMAIL || 'insights@flourish-mgmt.com';
      ...
      for (const subscriber of eligible) {
        try {
          ...
          // Crucial: Mark subscriber as sent for this monthKey so they are never emailed twice this month
