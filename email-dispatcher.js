...
class EmailDispatcher {
  ...
  getMonthArticles(targetDate = new Date()) {
    ...
    // Find all letters published in this specific month
    let matches = allPublished.filter(a => a.displayDate === currentMonthLabel);
    
    // If none found for this exact month (e.g. before first publication), take the latest month's published batch
    if (matches.length === 0 && allPublished.length > 0) {
      const latestMonth = allPublished[0].displayDate;
      matches = allPublished.filter(a => a.displayDate === latestMonth);
    }

    // Return full article objects
    return matches.map(m => (insightsEngine.articles && insightsEngine.articles.find(a => a.id === m.id)) || m);
  }

  extractChecklistHtml(bodyHtml) {
    if (!bodyHtml) return '';
    const listMatch = bodyHtml.match(/<(?:ol|ul)[^>]*>([\s\S]*?)<\/(?:ol|ul)>/i);
    if (!listMatch) return '';
  ...
  <div style="max-width: 640px; margin: 30px auto; background-color: #FAF7F2; border: 1px solid #DFD2C2; border-radius: 16px; overflow: hidden; box-shadow: 0 6px 20px rgba(0,0,0,0.04);">
    ...
    <div style="padding: 36px 30px;">
      ...
      <p style="font-size: 14px; line-height: 1.65; color: #4A5560; margin-bottom: 14px;">
        Capital allocators face a noisy market. Interest rate expectations shift weekly. Commercial property valuations are adjusting. Venture capital is returning to fundamentals. We don't guess market timing. We focus on real cash flow, physical asset quality, and strict downside protection.
      </p>
      <p style="font-size: 14px; line-height: 1.65; color: #4A5560; margin-bottom: 26px;">
        Each month, our General Partners synthesize these principles into direct, battle-tested partner letters designed to protect and compound generational capital. Below is your <strong>${monthLabel}</strong> executive brief.
      </p>
